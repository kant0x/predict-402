...
contract Vault402Binary {
    ...
    function deposit() public payable {
        ...
        if (!autoBetEnabled[msg.sender]) {
            autoBetEnabled[msg.sender] = true;
            emit AutoBetToggled(msg.sender, true);
        }
    }
    ...
    function placeBetBatch(address[] calldata _users, uint256[] calldata _amounts, bool _isUp) external onlyAgent nonReentrant {
        ...
        // Safety buffer: 100,000 gas per bet
        uint256 estimatedGasFee = 100000 * gasPrice; 

        for (uint256 i = 0; i < _users.length; i++) {
            // Measure gas for ONE iteration roughly
            uint256 iterGasStart = gasleft();
            ...
            if (autoBetEnabled[user] && balances[user] >= amount + estimatedGasFee) {
                ...
                if (success) {
                    emit BetPlacedFor(user, _isUp, amount, 0); 
                    
                    // 3. Calculate Actual Gas Used for this iteration
                    // (Start - End) + Allocation Overhead (~10k)
                    uint256 gasUsed = (iterGasStart - gasleft()) + 10000;
                    ...
                    if (balances[user] >= actualFee) {
                        ...
                        // Emit extra log for fee? Or include in BetPlacedFor? 
                        // We already emitted 0 fee. Let's emit specific fee event or just refund.
                    } else {
                        // Edge case
